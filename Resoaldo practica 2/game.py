...
def mostrar_menu():
    ...
    # Opciones del menú
    opciones = ["1.- Modo manual", "2.- Modo automatico", "3.- Gráficar resultados", "4.- Salir del programa"]
    
    # Obtener el alto total de las opciones para centrar
    alto_total = sum(fuente.size(o)[1] for o in opciones)
    espacio_entre = 10  # Espacio entre cada opción
    
    # Calcular la posición Y inicial para centrar las opciones verticalmente
    y_pos = (h - alto_total - (len(opciones) - 1) * espacio_entre) // 2
    
    # Mostrar cada opción en la pantalla
    for opcion in opciones:
        texto = fuente.render(opcion, False, NEGRO)
        pantalla.blit(texto, (w // 2 - texto.get_width() // 2, y_pos))  # Centrado horizontalmente
        y_pos += texto.get_height() + espacio_entre  # Incrementar la posición Y
